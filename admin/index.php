...
try {
    ...
    if ((int)$db->query('SELECT MAX(version) FROM cms_schema_versions')->fetchColumn() !== 1) throw new RuntimeException('Unsupported schema');
...
?>
<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MarryDJ · Administration</title><link rel="stylesheet" href="/admin/admin.css?v=1"></head><body>
<?php if (!$user): ?>
...
<?php if ($error): ?><p role="alert" class="notice"><?=cms_escape($error)?></p><?php endif ?>
<?php if ($page==='dashboard'): ?><p class="muted">Die Basis steht. Ihre Verwaltungsbereiche werden schrittweise ergänzt.</p><div class="cards"><?php foreach (['reviews'=>'Neue Bewertungen','weddings'=>'Hochzeiten & Gästecodes','inquiries'=>'Wichtige neue Anfragen'] as $id=>$label): ?><article><p class="eyebrow">IN VORBEREITUNG</p><h2><?=cms_escape($label)?></h2><p>Dieses Modul ist noch nicht aktiviert.</p><a href="/admin/?page=<?=$id?>">Bereich ansehen</a></article><?php endforeach ?></div><section class="notice"><h2>Sicher angemeldet</h2><p><?=cms_escape($user['email'])?></p><p>Die öffentliche Website und das Anfrageformular arbeiten weiterhin unverändert.</p></section>
